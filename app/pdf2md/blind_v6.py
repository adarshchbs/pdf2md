...
BUNDLE_SCHEMA_VERSION = "blind-review-v6.0.0"
ALGORITHM_VERSION = "independent-semantic-materiality-v6.0.0"
NORMALIZATION_VERSION = "neutral-contextual-evidence-v6.0.0"
ATTACK_VERSION = "descriptive-leakage-diagnostics-v6.0.0"
DESIGN_VERSION = "deterministic-blinding-invariants-v6.0.0"
...
def build_blind_v6(
    ...
    *,
    ...
    """Build a fresh v6 audit bundle using deterministic gates and staged publication."""
    ...
    # Pin every distinct evidence path referenced by the complete 425-record ledger,
    # not merely paths visible in the reviewer sample.
    ...
    adjudicability = _adjudicability_checks(sample_packets)
    design_acceptance = _deterministic_design_acceptance(sample_packets, sample_mappings, secret_seed)
    if adjudicability["result"] != "passed" or design_acceptance["result"] != "passed":
        raise ValueError(
            "v6 deterministic generation gates failed: "
            + json.dumps(
    ...
    attacks = {
        "reviewer_sample": _run_descriptive_attacks(sample_packets, sample_mappings),
        "full_material_population_diagnostic": _run_descriptive_attacks(full_packets, mappings),
        "acceptance_role": "diagnostic_only; no leakage acceptance decision is inferred",
        "statistical_power": "insufficient for the reviewer sample (n=6); exact intervals are reported",
    }

    packet_bytes = _pretty_bytes(sample_packets)
    audit = _audit_manifest(full_packets, sample_packets, mappings)
    ...
    manifest = cast(
        dict[str, JsonValue],
        {
            "artifact_id": "semantic13-human-audit-v6",
            "scope": "non_holdout",
            ...
            "algorithm_versions": {
                ...
                "descriptive_attacks": ATTACK_VERSION,
            ...
            "inputs": {
                ...
                "distinct_ledger_evidence_path_count": len(evidence_paths),
                "evidence_pin_scope": "every distinct path referenced by every full-ledger record",
            ...
            "adjudicability_checks": adjudicability,
            "deterministic_design_acceptance": design_acceptance,
            "descriptive_attacks": attacks,
            ...
            "separation": {
                ...
                "seed_map_colocation_limitation": (
                    "seed.json and source-map.json are co-located in the protected directory; "
                    "mode bits are defense-in-depth, and compromise of that directory reveals both"
                ),
                "publication_guarantee": (
                    "best-effort staged two-destination publication; all validation and gates finish "
                    "before publication, but two independent destination renames are not globally atomic"
                ),
            },
            "sha256": {
            ...
            "validation": {
                ...
                "reviewer_file_count_exactly_three": "passed",
                "protected_modes": "directory_0700_files_0600",
                "prepublication_failure_destinations": "none",
                "publication_model": "best_effort_staged_not_globally_atomic",
            },
        },
    )
    manifest_bytes = _pretty_bytes(manifest)
    source_map = {
        "artifact_id": "semantic13-protected-source-map-v6",
    ...
    seed_record = {
        "artifact_id": "semantic13-protected-seed-v6",
        "encoding": "hex",
    ...
    full_ledger = {
        "artifact_id": "semantic13-protected-full-ledger-v6",
...
def _with_neighbor_context(
    ...
    record["candidate_context"] = _neighbor_context(candidates, candidate_index)
    record["reference_context"] = _neighbor_context(references, reference_index)
    return record


def _neighbor_context(elements: list[DocumentElement], index: int | None) -> dict[str, JsonValue] | None:
    if index is None:
        return None

    def neighbor(position: int) -> dict[str, JsonValue] | None:
        ...
        return {
            ...
            "page_numbers": cast(JsonValue, sorted(_fragment_pages(element))),
        }
        ...
        if record["mode"] == "paired_ab":
            ...
            if "reading_order_segmentation" in cast(list[str], record["categories"]):
                ...
                packet["order_adjudication"] = {
                    "prompt": "Which neutral alternate placement has the correct reading order and segmentation?",
                    "assignment_a": candidate_context if candidate_first else reference_context,
                    "assignment_b": reference_context if candidate_first else candidate_context,
                    "context_window": "immediate previous and next elements; text truncated to 500 characters",
                    "provenance": "omitted",
                }
            mapping = {
                "packet_id": packet_id,
...
def _reviewer_sample(
    ...
    for mapping in mappings:
        name, population_count, quota = stratum_by_id[cast(str, mapping["packet_id"])]
...
def _category_is_adjudicable(
    ...
    if category == "reading_order_segmentation":
        ...
        return (
            packet["mode"] == "paired_ab"
            and isinstance(order, dict)
            and order.get("assignment_a") is not None
            and order.get("assignment_b") is not None
            and "candidate" not in order
            and "reference" not in order
        )
...
def _deterministic_design_acceptance(
    ...
    domain_keys = {
        domain: hmac.new(seed, f"blind-v6-key\0{domain}".encode(), hashlib.sha256).hexdigest()
...
def _audit_manifest(
    ...
    return cast(
        dict[str, JsonValue],
        {
            "artifact_id": "semantic13-secret-human-audit-sample-v6",
            "state": "pending_human_review",
            ...
            "sampling": {
                ...
                "inclusion_probabilities": [
                    ...
                    for stratum in sorted({cast(str, m["inclusion_stratum"]) for m in mappings})
                ],
...
def _implementation_pins() -> dict[str, JsonValue]:
    ...
    module_paths = {
        "app.pdf2md.blind_v6": Path(__file__),
        "app.pdf2md.evaluation": Path(__file__).with_name("evaluation.py"),
        "app.pdf2md.schema": Path(__file__).with_name("schema.py"),
        "app.pdf2md.tables": Path(__file__).with_name("tables.py"),
    }
    package_names = ("numpy", "pydantic", "pyarrow", "rapidfuzz", "scikit-learn", "scipy")
    ...
    config_paths = [project_root / "pyproject.toml", project_root / "uv.lock"]
    return cast(
        dict[str, JsonValue],
        {
            "runtime": {
            ...
            "full_module_sha256": {
                name: _sha256(path.read_bytes()) for name, path in sorted(module_paths.items())
            },
            "evaluator_dependency_closure": {
                "project_modules": sorted(module_paths),
                "packages": list(package_names),
                "closure_rule": "recursive direct imports used by evaluation, schema serialization, and v6 selection/diagnostics",
            },
            "package_distribution_pins": packages,
            "config_sha256": {
                path.relative_to(project_root).as_posix(): _sha256(path.read_bytes()) for path in config_paths
            },
            "schema_constants": {
                "document": SCHEMA_VERSION,
                "bundle": BUNDLE_SCHEMA_VERSION,
...
def _stage_and_publish_best_effort(
    ...
    reviewer_stage = reviewer_dir.with_name(f".{reviewer_dir.name}.v6-stage-{token}")
    protected_stage = protected_dir.with_name(f".{protected_dir.name}.v6-stage-{token}")
...
def _secret_token(seed: bytes, domain: str, value: str, *, length: int) -> str:
    key = hmac.new(seed, f"blind-v6-key\0{domain}".encode(), hashlib.sha256).digest()
