...
@njit
def line_distance(first_line: NDArray, second_line: NDArray) -> float:
    ...
    variable_cordinate_distance = variable_cordinate_concat[1] - variable_cordinate_concat[0]

    return numba_max([
        2 * constant_cordinate_distance,
        variable_cordinate_distance * is_range_not_intersecting(first_line[1:], second_line[1:]),
    ])


@njit
def combine_lines(lines: NDArray):
    constant_cordinate = np.int32(np.mean(lines[:, 0]))
    return np.array([constant_cordinate, np.min(lines[:, 1]), np.max(lines[:, 2])], dtype=np.int32)


def cluster_lines(lines: NDArray, line_type: LineType):
    ...
    unique_lines = [line_parameter_from_three_point_representation(line, line_type) for line in unique_lines]
    return unique_lines
